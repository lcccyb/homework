...
//十六进制输出密文
static char Hex(int input1, int input2, int input3, int input4) {
	int temp = input1 * pow(2, 3) + input2 * pow(2, 2) + input3 * 2 + input4;
...
static void DES_Efun(char input[8], char key_in[8], int output[64]) {
	...
	int key[64];
	int l[17][32], r[17][32];
	CharToBit(input, chartobit, 8);//转换为64个二进制数s  
...
int main() {
	...
	for (int i = 0; i < 64; i++) {
		if (i % 4 == 0) {
			printf("%c ", Hex(output[i], output[i + 1], output[i + 2], output[i + 3]));
		}
	}
	printf("\n\n");
	DES_Dfun(output, MI, JIEMIN); //解密
	printf("解密后明文如下:\n");
	for (int i = 0; i < 8; i++) {
		printf("%c", JIEMIN[i]);
	}
